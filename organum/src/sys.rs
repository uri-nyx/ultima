...
use crate::premade::bus::Bus;
use crate::debugger::Debugger;
use crate::error::{Error, ErrorType};
use crate::interrupts::InterruptController;
use crate::core::{Clock, ClockElapsed, Address, TransmutableBox};
...
impl System {
    ...
    pub fn get_data(&self) -> RefMut<'_, Bus> {
        self.bus_data.borrow_mut()
    }
    ...
    pub fn add_addressable_device_data(&mut self, addr: Address, device: TransmutableBox) -> Result<(), Error> {
        self.add_peripheral_data(&format!("mem{:x}", addr), addr, device)
    }
    ...
    pub fn add_peripheral_data(&mut self, name: &str, addr: Address, device: TransmutableBox) -> Result<(), Error> {
        self.bus_data.borrow_mut().insert(addr, device.clone());
        self.try_queue_device(device.clone());
        self.devices.insert(name.to_string(), device);
        Ok(())
    }
